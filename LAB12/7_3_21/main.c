...
void Lista_bez_Glowy_bezwzgledna(struct element*Lista)
{
    while(Lista!=NULL)
    {
        if(Lista->i<0)
...
void Lista_z_Glowa_bezwzgledna(struct element*Lista)
{
    while(Lista->next!=NULL)
    {
        if(Lista->next->i<0)
        {
            Lista->next->i=(-1)*Lista->next->i;
        }
        Lista=Lista->next;
    }
}
...
int main()
{
    ...
    Lista_bez_Glowy_bezwzgledna(lista1);
    Lista_bez_Glowy_wyswietl(lista1);
    ...
    Lista_z_Glowa_bezwzgledna(lista2);
    Lista_z_Glowa_wyswietl(lista2);
